...
module sparsemat
  ...
  interface operator(*)
     module procedure scalarMultSp, SpMultiplyVec
...
contains
  ...
  subroutine deleteblankspmat(Asp)
    ...
    double precision,      dimension( Asp%N_nonzero ) :: dummy_real
  ...
  !> multiplies a spmat to a vector but checks the input first
  !! then calls SpMultVec
  function SpMultiplyVec(Asp,vecX) result(vecY)
    double precision, intent(in) :: vecX(:)
    type(spmat),      intent(in) :: Asp

    double precision, allocatable :: vecY(:)
    ...
    allocate( VecY(Asp%N_i) )

    call SpMultVec(Asp,vecX,vecY)
    
  end function SpMultiplyVec
  
  !> multiplies a spmat to a vector
  subroutine SpMultVec(Asp, vecX, vecY)
    type(spmat),                          intent(in)    :: Asp
    double precision, dimension(Asp%N_j), intent(in)    :: vecX(:)
    double precision, dimension(Asp%N_i), intent(inout) :: vecY(:)

    ! this is for safty but might not be necessary.
    !double precision, dimension( Asp%N_j ) :: vecDummy
    ...
    do n=1, Asp%N_nonzero
       ...
       vecY(i) = vecY(i) + Asp%k(n) * vecX(j)
    enddo

  end subroutine SpMultVec
  ...
  subroutine assignsp2full(Bfull,Asp)
    type(spmat),                   intent(in)    :: Asp
    double precision, allocatable, intent(inout) :: Bfull(:,:)
  ...
  subroutine sum_redundant_fast(Asp,Bsp)
    type(spmat),           intent(in)    :: Asp
    type(spmat),           intent(inout) :: Bsp
